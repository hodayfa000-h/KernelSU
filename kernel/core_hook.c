...
{
    /* If KernelSU module isn’t mounted yet, nothing to unmount */
    if (!ksu_module_mounted)
        return 0;

    /* Validate credentials pointers */
    if (!new || !old)
        return 0;
    ...
    /* Only root→other transitions trigger unmount logic */
    if (old_uid.val != 0)
        return 0;

#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
    /* For susFS zygote mount handling, bypass unmount for system process */
    bool is_zygote_child = susfs_is_sid_equal(old->security, susfs_zygote_sid);
    if (likely(is_zygote_child)) {
        if (unlikely(new_uid.val >= 1000 && new_uid.val < 10000)) {
    ...
    /* Only handle real app UIDs that aren’t explicitly allowed */
...
#ifdef CONFIG_KSU_SUSFS
    /* Mark the task for susFS state tracking */
    task_lock(current);
...
out_ksu_try_umount:
    ...
    /* Fallback zygote check when SUS_MOUNT is disabled */
    {
        bool fallback_is_zygote = ksu_is_zygote(old->security);
        if (!fallback_is_zygote) {
    ...
    /* Skip if this UID doesn’t require unmounting */
